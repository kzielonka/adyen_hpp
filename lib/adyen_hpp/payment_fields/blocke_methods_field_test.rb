...
class AdyenHppPaymentFieldsBlockedMethodsFieldTest < Minitest::Test
  def setup
    @class = AdyenHpp::PaymentFields::BlockedMethodsField
    @field = @class.new
  end

  def test_field_name
    assert_equal 'blocked_methods', @class.field_name
  end

  def test_adyen_field_name
    assert_equal 'blockedMethods', @class.adyen_field_name
  end
